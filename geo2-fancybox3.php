<?php
/**
 * Function used to show the Fancybox 3 Lightbox with a typical Bing map.
 *
 ...
 * @since      2.0.6 Function geo2_maps_slimbox2_options() amended.
 ...
 * Function used to show the Fancybox 3 Lightbox with a typical Bing map.
 *
 * Similar to geo2_maps_fancybox3_options_worldmap() in geo2-fancybox3-worldmap.php.
 *
 * @since  2.0.0
 * @since  2.0.4 Capital letters removed from plugin $options keys.
 * @since  2.0.6 Variable $map_output defined instead of referred to with dot "$map_output. =...". Variable $pic_desc defined before it's used.
 * @since  2.0.7 Incorrect "closeBtn" option corrected to "smallBtn".
 *
 * @see    function geo2_maps() in bing-map.php
 ...
 */
function geo2_maps_fancybox3_options( $geo2_maps_lightbox, $picture_list, $options = null ) {
			$map_output = '	( function( $ ) {';
			// Removes last comma.
			$geo2_maps_lightbox = rtrim( $geo2_maps_lightbox, ',' );

			$map_output .= '
		$.fancybox3.open([ ' . $geo2_maps_lightbox . ' ], {';
		...
		$map_output .= '
			...
			caption : function( instance, item ) {
				...
				{ 
					...
					' : '
					if ( Object.keys( pic_desc[i] )[0] === item.opts.gid )
					{
						return pic_desc[i][item.opts.imageId];
					} else if ( Object.keys( pic_desc[i] )[0] === item.opts.aid ) {
						return pic_desc[i][item.opts.aid];
					}' ) . '
				}
  		},';
	}
